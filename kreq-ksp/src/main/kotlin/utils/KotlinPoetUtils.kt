 ...
 * 添加一条 return 语句
 */
fun FunSpec.Builder.buildReturnStatement(varName: String, withReturnKeyword: Boolean = true) {
    if (withReturnKeyword) {
        addStatement("return $varName")
    } else {
        addStatement(varName)
    }
...
fun FunSpec.Builder.setSuspend() = addModifiers(KModifier.SUSPEND)
